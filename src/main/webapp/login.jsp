	...
	<head>
		...
		<script type="text/javascript">
			...
			function login() {
				...
				$.ajax({
					url : "http://localhost:8080/university/login",
					type : "POST",
					data : data,
					dataType : "json",
			...
			function register() {
				...
				//关键的一步来了:将前端输入的数据发送给后台控制器，怎么发送？？？
				$.ajax({
					url : "http://localhost:8080/university/register",
					type : "POST",
					data : data,
					dataType : "json",
					success : function(data) {
...
<div style="margin: 0 auto;width: 30%;height: 50%;margin-top: 7%;border-radius: 12px;
			...
			<div style="width: 100%;margin-top: 48px;color: salmon;font-size: 30px;font-weight: bold;">登录</div>
			...
			<div style="margin-top: 18px;height: 35px;width: 100%;text-align: center;">
				<div onclick="login()" style="margin: 0 auto;height: 100%;line-height: 35px;width: 60%;text-align: center;background: salmon;border-radius: 5px;color: white;">
					登录
				</div>
				<div onclick="show_register_window()" style="font-size: 14px;padding: 10px;color: peachpuff;">
					没有账号？注册
				</div>
			</div>
		...
		<div id="register_window" style="position: absolute;width: 100%;height: 100%;background: rgba(0,0,0,0.5);top: 0;left: 0;display: none;">
			<div style="width: 30%;height: 70%;top: 15%;left: 35%;
				background: seashell;position: relative;
				border-radius: 12px;text-align: center;">
				<div style="margin: 0 auto;height: 80%;height: 90%;position: relative;top: 5%;
					padding-top: 5%;">
					<div style="width: 100%;color:salmon ;font-size: 30px;font-weight: bold;">注册</div>
					<div style="margin-top: 18px;height: 35px;width: 100%;">
					...
					<div style="margin-top: 25px;height: 35px;width: 100%;text-align: center;">
						<div onclick="register()" style="margin: 0 auto;height: 100%;line-height: 35px;width: 60%;text-align: center;background: salmon;border-radius: 5px;color: white;">
							注册
						</div>
						<div onclick="cancel_register()" style="margin: 0 auto;height: 100%;line-height: 35px;width: 60%;text-align: center;background: silver;border-radius: 5px;color: white;margin-top: 12px;">
							取消
						</div>
					</div>
				</div>
